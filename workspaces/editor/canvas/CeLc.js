        ...
        for (uid in lcParentMap) {
          ...
          svgEl = this.createNode({
            ...
            svg.image(MC.IMG_URL + this.iconUrl(), 39, 27).move(27, 15).classes("ami-image"), svg.use("port_diamond").move(10, 20).attr({
              ...
              'data-tooltip': lang.ide.PORT_TIP_D
            }), svg.image(MC.IMG_URL + "ide/icon/instance-volume.png", 29, 24).move(31, 46).classes('volume-image'), svg.plain("").move(45, 58).classes('volume-number'), svg.group().add([svg.rect(20, 14).move(36, 2).radius(3).classes("server-number-bg"), svg.plain("0").move(46, 13).classes("server-number")]).classes("server-number-group")
