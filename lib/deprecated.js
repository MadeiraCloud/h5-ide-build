(function() {
  ...
  define('lib/aws/aws',['MC', 'constant', 'underscore', 'jquery', 'Design'], function(MC, constant, _, $, Design) {
    var cacheResource, checkAppName, checkIsRepeatName, checkPrivateIPIfHaveEIP, checkStackName, collectReference, convertBlockDeviceMapping, enableStackAgent, genAttrRefList, genResRef, getCompByResIdForState, getDuplicateName, getOSFamily, getStackNameFromApp, hasStack, isValidInIPRange, replaceReference;
    ...
    checkStackName = function(stackId, newName) {
      var isInTab, stackArray;
      isInTab = false;
      _.each(MC.tab, function(tabData) {
        if (!isInTab && (tabData.design_model.mode() === "stack") && (tabData.data.id !== stackId) && (tabData.data.name === newName)) {
          isInTab = true;
        }
        return null;
      });
    ...
    hasStack = function(stackId) {
      var stackArray, targetStack;
      stackArray = _.flatten(_.values(MC.data.stack_list));
      targetStack = _.find(stackArray, function(stack) {
    ...
    getStackNameFromApp = function(app_name) {
      var copy_name, i, idx, name_list, prefix, reg_name, stack_reg, stacks, _i, _len;
      if (!app_name) {
        app_name = "untitled";
      }
      idx = 0;
    ...
    collectReference = function(canvas_component) {
      ...
      for (uid in canvas_component) {
        ...
        if (constant.AWS_RESOURCE_KEY[comp.type] && comp.resource[constant.AWS_RESOURCE_KEY[comp.type]]) {
  ...
  define('forge_handle',['MC', 'constant'], function(MC, constant) {
    ...
    getAmis = function(data) {
      ...
      _.each(data.component, function(item) {
        var _ref, _ref1;
        if (item.type === 'AWS.EC2.Instance' && item.resource && item.resource.ImageId && !(_ref = item.resource.ImageId, __indexOf.call(amis, _ref) >= 0)) {
          amis.push(item.resource.ImageId);
        }
        if (item.type === 'AWS.AutoScaling.LaunchConfiguration' && item.resource && item.resource.ImageId && !(_ref1 = item.resource.ImageId, __indexOf.call(amis, _ref1) >= 0)) {
...
(function() {
  define('lib/common/convert',['MC', 'constant', 'underscore', 'jquery'], function(MC, constant, _, $) {
    var convertACL, convertASG, convertAZ, convertCGW, convertDHCP, convertEIP, convertELB, convertEni, convertIGW, convertInstance, convertKP, convertLC, convertNC, convertRTB, convertSGGroup, convertScalingPolicy, convertSubnet, convertVGW, convertVPC, convertVPN, convertVolume, mapProperty, removeAppId, resolveEC2Tag, resourceId2CompUid;
    ...
    convertEni = function(aws_eni) {
      var eni_json, group, ip, tag, _i, _j, _len, _len1, _ref, _ref1, _ref2;
      eni_json = {
        "uid": MC.guid(),
      ...
      if (aws_eni.attachment && aws_eni.attachment.instanceOwnerId && ((_ref = aws_eni.attachment.instanceOwnerId) === "amazon-elb" || _ref === "amazon-rds")) {
        return false;
      }
      if (aws_eni.tagSet) {
      ...
      if (aws_eni.attachment) {
        ...
        if (aws_eni.attachment.deviceIndex !== "0") {
      ...
      _ref1 = aws_eni.privateIpAddressesSet.item;
    ...
    convertInstance = function(aws_instance, default_kp) {
      var instance_json, tag;
      instance_json = {
        "uid": MC.guid(),
      ...
      if (aws_instance.monitoring && aws_instance.monitoring) {
      ...
      if (default_kp && default_kp.resource && aws_instance.keyName === default_kp.resource.KeyName) {
    ...
    convertELB = function(aws_elb) {
      ...
      elb_json.name = this.removeAppId(aws_elb.LoadBalancerName);
      if (elb_json.name !== aws_elb.LoadBalancerName) {
    ...
    convertACL = function(aws_acl) {
      var acl, acl_json, tag, _i, _j, _len, _len1, _ref, _ref1;
      acl_json = {
        "uid": MC.guid(),
    ...
    convertRTB = function(aws_rtb) {
      var asso, prop, route, rtb_json, tag, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
      rtb_json = {
        "uid": MC.guid(),
    ...
    convertVPC = function(aws_vpc, vpc_attr_data) {
      var tag, vpc_json;
      vpc_json = {
        "uid": MC.guid(),
        "type": "AWS.VPC.VPC",
    ...
    convertVolume = function(aws_vol, region) {
      var deviceName, instanceId, res_cache, rootDeviceName, vol_json;
      vol_json = {
        "uid": MC.guid(),
      ...
      if (aws_vol.attachmentSet) {
        if (MC.data.resource_list[region]) {
          ...
          if (res_cache[instanceId]) {
            ...
            if (rootDeviceName.indexOf(deviceName) === 0) {
              return null;
            }
          }
        }
    ...
    convertVPN = function(aws_vpn) {
      var route, tag, vpn_json, _i, _len, _ref;
      vpn_json = {
        "uid": MC.guid(),
    ...
    convertDHCP = function(aws_dhcp) {
      var dhcp, dhcp_json, tag, value, valueset, _i, _j, _len, _len1, _ref, _ref1;
      dhcp_json = {
        "uid": MC.guid(),
    ...
    resolveEC2Tag = function(tagSet) {
      ...
      if (tagSet) {
        result = {
          "Created by": tagSet["Created by"] ? tagSet["Created by"] : '',
          "Name": tagSet["Name"] ? tagSet["Name"] : '',
          "app": tagSet["app"] ? tagSet["app"] : '',
          "app-id": tagSet["app-id"] ? tagSet["app-id"] : '',
          "name": tagSet["name"] ? tagSet["name"] : '',
          "isApp": false
        };
        if (tagSet["Created by"] && tagSet["Name"] && tagSet["app"] && tagSet["app-id"]) {
          result.isApp = true;
        }
      }
      return result;
    };
    removeAppId = function(name) {
      var reg_name;
      reg_name = /.*app-[a-z0-9]{8}$/;
      if (reg_name.test(name)) {
        return name.substr(0, name.lastIndexOf("-app-"));
      } else {
        return name;
      }
    };
    resourceId2CompUid = function(components) {
      var comp, resId, resKey, result, uid;
      result = {};
      if (components) {
...
(function() {
  define('common_handle',['MC', 'lib/common/other', 'lib/common/convert'], function(MC, other, convert_handler) {
    MC.common = {
      other: other,
