(function() {
  ...
  define('lib/aws/aws',['MC', 'constant', 'underscore', 'jquery', 'Design'], function(MC, constant, _, $, Design) {
    var cacheResource, checkAppName, checkDefaultVPC, checkIsRepeatName, checkPrivateIPIfHaveEIP, checkStackName, collectReference, convertBlockDeviceMapping, enableStackAgent, genAttrRefList, genResRef, getCompByResIdForState, getDuplicateName, getOSFamily, isValidInIPRange, replaceReference;
    ...
    getCompByResIdForState = function(resId) {
      ...
      Design.instance().eachComponent(function(component) {
        ...
        if (component.get('appId') === resId) {
          ...
          if (component.type === constant.AWS_RESOURCE_TYPE.AWS_AutoScaling_LaunchConfiguration) {
    ...
    checkPrivateIPIfHaveEIP = function(allCompData, eniUID, priIPNum) {
      ...
      _.each(allCompData, function(compData) {
        ...
        if (compData.type === constant.AWS_RESOURCE_TYPE.AWS_EC2_EIP) {
    ...
    genAttrRefList = function(currentCompData, allCompData) {
      ...
      currentIsASG = false;
      currentASGName = null;
      if (currentCompType === constant.AWS_RESOURCE_TYPE.AWS_AutoScaling_LaunchConfiguration) {
        currentIsASG = true;
      }
      currentIsISG = false;
      ...
      _.each(allCompData, function(compData, uid) {
        ...
        if (compType === constant.AWS_RESOURCE_TYPE.AWS_AutoScaling_Group) {
          ...
          if (lcUIDRef) {
            ...
            if (currentCompType === constant.AWS_RESOURCE_TYPE.AWS_AutoScaling_LaunchConfiguration && currentCompUID === lcUID) {
...
(function() {
  define('forge_handle',['MC', 'constant', 'Design'], function(MC, constant, Design) {
...
(function() {
  define('lib/common/cookie',['jquery', 'MC', 'constant'], function($, MC, constant) {
    var checkAllCookie, clearV2Cookie, deleteCookie, getCookieByName, setCookie, setCookieByName, setCred, setIDECookie;
    ...
    deleteCookie = function() {
      ...
      return $.cookie('is_invitated', '', option);
    };
    ...
    setIDECookie = function(result) {
      ...
      madeiracloud_ide_session_id = [result.usercode, result.email, result.session_id, result.account_id, result.mod_repo, result.mod_tag, result.state, result.has_cred, result.is_invitated];
      return null;
    };
    checkAllCookie = function() {
      if ($.cookie('usercode') && $.cookie('username') && $.cookie('session_id') && $.cookie('account_id') && $.cookie('mod_repo') && $.cookie('mod_tag') && $.cookie('state') && $.cookie('has_cred') && $.cookie('is_invitated')) {
        return true;
      } else {
        return false;
      }
