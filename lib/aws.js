(function() {
  ...
  define(['MC', 'constant', 'underscore', 'jquery', 'Design', 'i18n!/nls/lang.js', "OpsModel"], function(MC, constant, _, $, Design, lang, OpsModel) {
    ...
    genAttrRefList = function(currentCompData, allCompData) {
      var allAttrStrAry, allListenerModels, allPortModels, allSubnetModels, autoCompList, awsPropertyData, currentASGName, currentCompType, currentCompUID, currentISGName, currentInstanceName, currentIsASG, currentIsISG, currentIsInstance, currentResModel, groupAutoCompList, instanceAutoCompList, lcUID, lcUIDRef, resAttrDataAry, _genRefObj, _getSelectedASGModelByLC;
      resAttrDataAry = [];
      if (Design.instance().type() !== OpsModel.Type.OpenStack) {
        _getSelectedASGModelByLC = function() {
          ...
          if (asgViewId) {
            return App.sceneManager.activeScene().getAwakeSpace().view.canvas.getItem(asgViewId).model;
          }
          return null;
        };
        ...
        _genRefObj = function(name, uid, attr) {
          return {
            name: "" + name + "." + attr,
            ref: "" + uid + "." + attr,
            uid: "" + uid + "." + attr,
        ...
        _.each(allPortModels, function(portModel) {
          var floatIPModel, name, uid, _ref;
          name = portModel.get('name');
          if (portModel.isEmbedded()) {
            name = (_ref = portModel.owner()) != null ? _ref.get('name') : void 0;
          }
          uid = portModel.id;
          if (!(name && uid)) {
            return;
          }
          resAttrDataAry = resAttrDataAry.concat([_genRefObj(name, uid, 'ip_address'), _genRefObj(name, uid, 'mac_address')]);
          floatIPModel = portModel.getFloatingIp();
          if (floatIPModel) {
            uid = floatIPModel.id;
            return resAttrDataAry = resAttrDataAry.concat([_genRefObj(name, uid, 'public_ip')]);
          }
        });
        allListenerModels = Design.modelClassForType(constant.RESTYPE.OSLISTENER).allObjects();
        _.each(allListenerModels, function(listenerModel) {
          var floatIPModel, name, uid;
          name = listenerModel.get('name');
          uid = listenerModel.id;
          resAttrDataAry = resAttrDataAry.concat([_genRefObj(name, uid, 'ip_address'), _genRefObj(name, uid, 'mac_address'), _genRefObj(name, uid, 'protocol'), _genRefObj(name, uid, 'port')]);
        ...
        _.each(allSubnetModels, function(subnetModel) {
          var name, uid;
          name = subnetModel.get('name');
          uid = subnetModel.id;
          return resAttrDataAry = resAttrDataAry.concat([_genRefObj(name, uid, 'cidr')]);
        });
      }
