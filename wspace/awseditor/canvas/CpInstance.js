  ...
  define(["CanvasPopup", "./TplPopup", "./CpVolume", "event", "constant", "CloudResources"], function(CanvasPopup, TplPopup, VolumePopup, ide_event, constant, CloudResources) {
    return CanvasPopup.extend({
      ...
      showVolume: function(evt) {
        ...
        ins = CloudResources(this.canvas.design.credentialId(), constant.RESTYPE.INSTANCE, region).get($ins.attr("data-id"));
        if (!ins) {
          return;
        }
        ins = ins.attributes;
        volCln = CloudResources(this.canvas.design.credentialId(), constant.RESTYPE.VOL, region);
        vols = [];
