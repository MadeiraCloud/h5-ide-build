...
helpers = this.merge(helpers, Handlebars.helpers); data = data || {};
  ...
  buffer += "<button id=\"saveAndCloseBtn\" data-pending=\""
    + escapeExpression(helpers.i18n.call(depth0, "TOOLBAR.SAVING_BTN", {hash:{},data:data}))
    + "\" class=\"btn btn-blue\">"
    + escapeExpression(helpers.i18n.call(depth0, "TOOLBAR.SAVE_AND_CLOSE_BTN", {hash:{},data:data}))
    + "</button>";
  return buffer;
  };
TEMPLATE.modal.saveAndCloseBtn =Handlebars.template(__TEMPLATE__);
  ...
  define('CoreEditorView',["wspace/coreeditor/TplOpsEditor", "UI.modalplus", "i18n!/nls/lang.js", "AppAction", "backbone", "UI.selectbox", "backbone", "UI.selectbox"], function(OpsEditorTpl, Modal, lang, AppAction) {
    ...
    return Backbone.View.extend({
      ...
      showCloseConfirm: function() {
        ...
        $(OpsEditorTpl.modal.saveAndCloseBtn()).prependTo(modal.$(".modal-footer")).click(function() {
          var saveIcon;
          saveIcon = $("#OpsEditor .icon-save");
          modal.setContent(MC.template.loadingSpinner()).setTitle(lang.IDE.SAVING_STACK).$(".modal-footer").hide().end().find(".modal-header .modal-close").off("click");
          return new AppAction({
            workspace: self.workspace
          }).saveStack(saveIcon, self).then(function() {
            modal.close();
