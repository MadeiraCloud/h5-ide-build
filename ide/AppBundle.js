  ...
  define('OpsModel',["ApiRequest", "constant", "CloudResources", "ThumbnailUtil", "backbone"], function(ApiRequest, constant, CloudResources, ThumbUtil) {
    ...
    OpsModel = Backbone.Model.extend({
      ...
      generateJsonFromRes: function() {
        var asg, asgCompAry, comp, component_data, invalidExpandedAsgAry, invalidUid, key, layout, layout_data, subnet, subnetMap, subnetRefAry, subnetUidAry, tmpAry, topic, topicComp, topicCompAry, topicMap, uid, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m;
        component_data = CloudResources('OpsResource', this.getVpcId()).generatedJson.component;
        layout_data = CloudResources('OpsResource', this.getVpcId()).generatedJson.layout;
        topicMap = {};
        topicCompAry = [];
        asgCompAry = [];
        for (key in component_data) {
          comp = component_data[key];
          if (comp.type === constant.RESTYPE["ASG"]) {
            asgCompAry.push(comp);
          } else if (comp.type === constant.RESTYPE["NC"] && comp.resource.TopicARN.indexOf("arn:aws:sns:") === 0) {
        ...
        for (_i = 0, _len = topicCompAry.length; _i < _len; _i++) {
          topic = topicCompAry[_i];
        ...
        for (key in layout_data) {
          ...
          if (layout.type && layout.type === 'ExpandedAsg' && layout.originalId) {
            if (!component_data[layout.originalId]) {
              invalidExpandedAsgAry.push(key);
            }
          }
        }
        for (_j = 0, _len1 = invalidExpandedAsgAry.length; _j < _len1; _j++) {
          invalidUid = invalidExpandedAsgAry[_j];
          console.warn("remove invalid ExpandedAsg");
          delete layout_data[invalidUid];
        }
        for (_k = 0, _len2 = asgCompAry.length; _k < _len2; _k++) {
          asg = asgCompAry[_k];
          subnetMap = {};
          subnetMap[layout_data[asg.uid].groupUId] = true;
          subnetRefAry = asg.resource.VPCZoneIdentifier.split(" , ");
          subnetUidAry = [];
          for (_l = 0, _len3 = subnetRefAry.length; _l < _len3; _l++) {
            subnet = subnetRefAry[_l];
            subnetUidAry.push(MC.extractID(subnet));
          }
          for (key in layout_data) {
            layout = layout_data[key];
            if (layout.originalId === asg.uid && !layout.groupUId) {
              for (_m = 0, _len4 = subnetUidAry.length; _m < _len4; _m++) {
                subnet = subnetUidAry[_m];
                if (!subnetMap[subnet]) {
