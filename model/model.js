...
(function() {
  define('account_model',['backbone', 'underscore', 'account_service', 'base_model'], function(Backbone, _, account_service, base_model) {
    ...
    AccountModel = Backbone.Model.extend({
      ...
      register: function(src, username, password, email) {
        var me;
        me = this;
        src.model = me;
        return account_service.register(src, username, password, email, function(forge_result) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('ACCOUNT_REGISTER_RETURN', forge_result);
          }
        });
      },
      ...
      reset_password: function(src, username) {
        var me;
        me = this;
        src.model = me;
        return account_service.reset_password(src, username, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('ACCOUNT_RESET__PWD_RETURN', forge_result);
          }
        });
      },
      update_password: function(src, id, new_pwd) {
        var me;
        me = this;
        src.model = me;
        return account_service.update_password(src, id, new_pwd, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('ACCOUNT_UPDATE__PWD_RETURN', forge_result);
          }
        });
      },
      check_repeat: function(src, username, email) {
        var me;
        me = this;
        src.model = me;
        return account_service.check_repeat(src, username, email, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('ACCOUNT_CHECK__REPEAT_RETURN', forge_result);
          }
        });
      },
      check_validation: function(src, key, flag) {
        var me;
        me = this;
        src.model = me;
        return account_service.check_validation(src, key, flag, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('ACCOUNT_CHECK__VALIDATION_RETURN', forge_result);
          }
        });
      },
      ...
      is_invitated: function(src, username, session_id) {
        var me;
        me = this;
        src.model = me;
        return account_service.is_invitated(src, username, session_id, function(forge_result) {
          if (!forge_result.is_error) {
            if (src.sender && src.sender.trigger) {
              return src.sender.trigger('USER_IS__INVITATED_RETURN', forge_result);
            }
          } else {
      ...
      apply_trial: function(src, username, session_id, message) {
        var me;
        if (message == null) {
          message = null;
        }
        me = this;
        src.model = me;
        return account_service.apply_trial(src, username, session_id, message, function(forge_result) {
          if (!forge_result.is_error) {
            if (src.sender && src.sender.trigger) {
              return src.sender.trigger('USER_APPLY__TRIAL_RETURN', forge_result);
            }
          } else {
...
(function() {
  define('session_service',['MC', 'constant', 'result_vo'], function(MC, constant, result_vo) {
    var URL, guest, login, logout, parserGuestReturn, parserLoginReturn, parserLogoutReturn, parserSetCredentialReturn, parserSyncRedisReturn, resolveGuestResult, resolveLoginResult, resolveLogoutResult, resolveSetCredentialResult, resolveSyncRedisResult, send_request, set_credential, sync_redis;
    URL = '/session/';
    send_request = function(api_name, src, param_ary, parser, callback) {
      var error;
      if (callback === null) {
    ...
    resolveLoginResult = function(result) {
      var session_info;
      session_info = {};
      session_info.usercode = result[0];
      session_info.email = result[1];
    ...
    parserLoginReturn = function(result, return_code, param) {
      var forge_result, resolved_data;
      forge_result = result_vo.processForgeReturnHandler(result, return_code, param);
      if (return_code === constant.RETURN_CODE.E_OK && !forge_result.is_error) {
    ...
    resolveLogoutResult = function(result) {};
    parserLogoutReturn = function(result, return_code, param) {
      var forge_result, resolved_data;
      forge_result = result_vo.processForgeReturnHandler(result, return_code, param);
      if (return_code === constant.RETURN_CODE.E_OK && !forge_result.is_error) {
    ...
    resolveSetCredentialResult = function(result) {};
    parserSetCredentialReturn = function(result, return_code, param) {
      var forge_result, resolved_data;
      forge_result = result_vo.processForgeReturnHandler(result, return_code, param);
      if (return_code === constant.RETURN_CODE.E_OK && !forge_result.is_error) {
    ...
    resolveGuestResult = function(result) {
      var session_info;
      session_info = {};
    ...
    parserGuestReturn = function(result, return_code, param) {
      var forge_result, resolved_data;
      forge_result = result_vo.processForgeReturnHandler(result, return_code, param);
      if (return_code === constant.RETURN_CODE.E_OK && !forge_result.is_error) {
    ...
    resolveSyncRedisResult = function(result) {};
    parserSyncRedisReturn = function(result, return_code, param) {
      var forge_result, resolved_data;
      forge_result = result_vo.processForgeReturnHandler(result, return_code, param);
      if (return_code === constant.RETURN_CODE.E_OK && !forge_result.is_error) {
    ...
    login = function(src, username, password, callback) {
      send_request("login", src, [username, password], parserLoginReturn, callback);
      return true;
    };
    logout = function(src, username, session_id, callback) {
      send_request("logout", src, [username, session_id], parserLogoutReturn, callback);
      return true;
    };
    set_credential = function(src, username, session_id, access_key, secret_key, account_id, callback) {
      if (account_id == null) {
        account_id = null;
      }
    ...
    guest = function(src, guest_id, guestname, callback) {
      send_request("guest", src, [guest_id, guestname], parserGuestReturn, callback);
      return true;
    };
    sync_redis = function(src, username, session_id, callback) {
...
(function() {
  define('session_model',['backbone', 'underscore', 'session_service', 'base_model'], function(Backbone, _, session_service, base_model) {
    var SessionModel, session_model;
    SessionModel = Backbone.Model.extend({
      initialize: function() {
        return _.extend(this, base_model);
      },
      login: function(src, username, password) {
        var me;
        me = this;
        src.model = me;
        return session_service.login(src, username, password, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('SESSION_LOGIN_RETURN', forge_result);
          } else {
            return me.trigger('SESSION_LOGIN_RETURN', forge_result);
          }
        });
      },
      logout: function(src, username, session_id) {
        var me;
        me = this;
        src.model = me;
        return session_service.logout(src, username, session_id, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('SESSION_LOGOUT_RETURN', forge_result);
          }
        });
      },
      set_credential: function(src, username, session_id, access_key, secret_key, account_id) {
        var me;
        if (account_id == null) {
          account_id = null;
        }
        me = this;
        src.model = me;
        return session_service.set_credential(src, username, session_id, access_key, secret_key, account_id, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('SESSION_SET__CREDENTIAL_RETURN', forge_result);
          }
        });
      },
      sync_redis: function(src, username, session_id) {
        var me;
        me = this;
        src.model = me;
        return session_service.sync_redis(src, username, session_id, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('SESSION_SYNC__REDIS_RETURN', forge_result);
          }
        });
      },
      guest: function(src, guest_id, guestname) {
        var me;
        me = this;
        src.model = me;
        return session_service.guest(src, guest_id, guestname, function(forge_result) {
          if (!forge_result.is_error) {
          ...
          if (src.sender && src.sender.trigger) {
            return src.sender.trigger('SESSION_GUEST_RETURN', forge_result);
          }
        });
      }
    });
