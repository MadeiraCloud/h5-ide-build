...
(function() {
  define('module/design/framework/canvasview/CanvasElement',["CanvasManager", "event", "constant", "i18n!nls/lang.js", "MC.canvas.constant"], function(CanvasManager, ide_event, constant, lang) {
    ...
    CanvasElement.createView = function(type, model) {
      var CEC, m;
    ...
    CanvasElement.prototype.list = function() {
      ...
      id = this.id;
      name = this.model.get("name");
      resource_list = MC.data.resource_list[this.model.design().region()];
    ...
    CanvasElement.prototype.updateAppState = function() {
      ...
      if (!MC.data.resource_list[design.region()][m.get("appId")]) {
    ...
    CanvasElement.prototype.updatexGWAppState = function() {
      ...
      if (data) {
        if (m.get("appId").indexOf("igw-") === 0) {
          if (!(data.attachmentSet !== null && data.attachmentSet.item[0].state === "available" && data.attachmentSet.item[0].vpcId === m.parent().get("appId"))) {
            CanvasManager.addClass(el, "deleted");
          }
        } else if (m.get("appId").indexOf("vgw-") === 0) {
          if (!(data.state === "available" && data.attachments.item[0].state === "attached" && data.attachments.item[0].vpcId === m.parent().get("appId"))) {
            CanvasManager.addClass(el, "deleted");
          }
...
(function() {
  define('module/design/framework/canvasview/CanvasAdaptor',["./CanvasElement", "event", 'i18n!nls/lang.js', "constant"], function(CanvasElement, ide_event, lang, constant) {
...
(function() {
  ...
  define('Design',["constant", "module/design/framework/canvasview/CanvasAdaptor"], function(constant, CanvasAdaptor) {
     ...
     */
    Design = function(canvas_data, options) {
      var design, oldLayout;
    ...
    DesignImpl = function(canvas_data, options) {
      ...
      if ((canvas_data.version || "").split("-").length < 3) {
        this.attributes.version = "2013-09-13";
      }
      this.__shoulddraw = false;
      this.save(canvas_data);
    ...
    DesignImpl.prototype.typeIsClassic = function() {
      return this.attributes.platform === Design.TYPE.Classic;
    };
    DesignImpl.prototype.typeIsDefaultVpc = function() {
      return this.attributes.platform === Design.TYPE.DefaultVpc;
    };
    DesignImpl.prototype.typeIsVpc = function() {
      return this.attributes.platform === Design.TYPE.Vpc || this.attributes.platform === Design.TYPE.CustomVpc;
    };
    ...
    DesignImpl.prototype.save = function(canvas_data) {
      this.__backingStore = canvas_data ? canvas_data : this.serialize();
    };
    DesignImpl.prototype.isModified = function(newData) {
      ...
      _ref = newData || this.attributes;
      for (value = _i = 0, _len = _ref.length; _i < _len; value = ++_i) {
        key = _ref[value];
        if (!_.isEqual(this.__backingStore[key], value)) {
          return false;
        }
      }
      if (!newData) {
        newData = this.serialize();
        if (_.isEqual(this.__backingStore.component, newData.component)) {
          if (_.isEqual(this.__backingStore.layout, newData.layout)) {
            return false;
          }
        }
      }
    ...
    DesignImpl.prototype.getCost = function() {
      ...
      costList = [];
      totalFee = 0;
      feeMap = MC.data.config[this.region()];
      if (feeMap && feeMap.price) {
    ...
    DesignImpl.prototype.diff = function() {
      ...
      return {
        result: result,
        isRunning: newData.state === constant.APP_STATE.APP_STATE_RUNNING,
        isModified: isModified || !_.isEqual(oldData.layout, newData.layout)
      };
    };
    ...
    DesignImpl.prototype.clearResourceInCache = function() {
      var idx, lcList, resource_list, subList;
      resource_list = MC.data.resource_list[this.region()];
      if (!resource_list) {
        return;
      }
      this.eachComponent(function(comp) {
        var appId, key, member, val, _i, _len;
        appId = comp.get("appId");
        if (appId && appId.indexOf(":autoScalingGroup:") > 0 && resource_list[appId]) {
          member = resource_list[appId].Instances;
          if (member.member) {
            member = member.member;
          }
          for (key = _i = 0, _len = member.length; _i < _len; key = ++_i) {
            val = member[key];
            if (_.isString(val)) {
        ...
        return delete resource_list[appId];
      });
...
(function() {
  define('module/design/framework/ResourceModel',["Design", "event", "backbone"], function(Design, ideEvent) {
    ...
    ResourceModel = Backbone.Model.extend({
      ...
      hasAppResource: function() {
        if (!Design.instance().modeIsStack() && this.get("appId")) {
          return !!this.get("appId") && MC.data.resource_list[Design.instance().region()][this.get("appId")];
        } else {
          return true;
        }
      },
      ...
      createRef: function(refName, isResourceNS, id) {
        ...
        if (isResourceNS !== false) {
          return MC.aws.aws.genResRef(id, "resource." + refName);
        } else {
          return MC.aws.aws.genResRef(id, "" + refName);
        }
      },
...
(function() {
  define('module/design/framework/ConnectionModel',["./ResourceModel", "Design", "CanvasManager", "./canvasview/CanvasElement"], function(ResourceModel, Design, CanvasManager, CanvasElement) {
    ...
    ConnectionModel = ResourceModel.extend({
      ...
      getCanvasView: function() {
        ...
        if (this.__view === void 0) {
          this.__view = CanvasElement.createView("Line", this);
        }
        return this.__view;
      },
      isVisual: function() {
...
(function() {
  define('module/design/framework/connection/EniAttachment',["constant", "../ConnectionModel", "i18n!nls/lang.js"], function(constant, ConnectionModel, lang) {
    var C;
...
(function() {
  define('module/design/framework/ComplexResModel',["Design", "CanvasManager", "./ResourceModel", "constant", "./canvasview/CanvasElement"], function(Design, CanvasManager, ResourceModel, constant, CanvasElement) {
    ...
    ComplexResModel = ResourceModel.extend({
      ...
      parent: function() {
        return this.attributes.__parent || null;
      },
      x: function() {
...
(function() {
  ...
  define('module/design/framework/resource/InstanceModel',["../ComplexResModel", "Design", "constant", "i18n!nls/lang.js"], function(ComplexResModel, Design, constant, lang) {
    ...
    Model = ComplexResModel.extend({
      ...
      getDetailedOSFamily: function() {
        var ami, desc, imgLocation, name, osFamily, osType, sql_standerd_pattern, sql_web_pattern;
        ...
        if (__indexOf.call(constant.WINDOWS, osType) >= 0) {
          ...
          imgLocation = ami.imageLocation || "";
          if (name.match(sql_web_pattern) || desc.match(sql_web_pattern) || imgLocation.match(sql_web_pattern)) {
            osFamily = 'mswinSQLWeb';
          } else if (name.match(sql_standerd_pattern) || desc.match(sql_standerd_pattern) || imgLocation.match(sql_standerd_pattern)) {
            osFamily = 'mswinSQL';
          }
        }
        return osFamily;
      ...
      initInstanceType: function() {
        var ami, i, instance_type, type, _i, _len;
        ami = this.getAmi();
        if (ami && ami.instance_type) {
          instance_type = ami.instance_type.replace(/\s/g, "").split(",");
          for (_i = 0, _len = instance_type.length; _i < _len; _i++) {
            i = instance_type[_i];
            if (i !== "t1.micro") {
              type = i;
              break;
      ...
      getCost: function(priceMap, currency) {
        ...
        osFamily = this.getDetailedOSFamily();
      ...
      getInstanceTypeConfig: function(type) {
        var config, t;
        t = (type || this.get("instanceType")).split(".");
        if (t.length >= 2) {
          config = MC.data.config[Design.instance().region()];
          if (config && config.instance_type) {
      ...
      isEbsOptimizedEnabled: function() {
        var EbsMap, ami, instanceType, k;
        ami = this.getAmi() || this.get("cachedAmi");
        if (ami && ami.rootDeviceType === "instance-store") {
          return false;
        }
        k = this.get("instanceType").split('.');
        instanceType = MC.data.config[Design.instance().region()].instance_type;
        if (instanceType) {
          instanceType = instanceType[k[0]];
        }
        if (instanceType) {
          instanceType = instanceType[k[1]];
        }
      ...
      getInstanceTypeList: function() {
        var instanceType, instance_type_list, tenancy;
        instance_type_list = MC.aws.ami.getInstanceType(this.getAmi());
        if (!instance_type_list) {
          return [];
        }
        tenancy = this.isDefaultTenancy();
        instanceType = this.get("instanceType");
        return _.map(instance_type_list, function(value) {
          return {
      ...
      isRemovable: function() {
        ...
        if ((state !== void 0 && state.length > 0) || ($('#state-editor-model').is(':visible') && $('#state-editor-model .state-list .state-item').length >= 1)) {
...
(function() {
  define('module/design/framework/connection/SgAsso',["constant", "../ConnectionModel", "CanvasManager", "Design"], function(constant, ConnectionModel, CanvasManager, Design) {
    var SgAsso;
...
(function() {
  define('module/design/framework/resource/VolumeModel',["i18n!nls/lang.js", "../ComplexResModel", "constant"], function(lang, ComplexResModel, constant) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: {
        name: '',
...
(function() {
  define('module/design/framework/resource/AclModel',["../ComplexResModel", "../ConnectionModel", "constant"], function(ComplexResModel, ConnectionModel, constant) {
...
(function() {
  define('module/design/framework/GroupModel',["Design", "./ComplexResModel"], function(Design, ComplexResModel) {
    var GroupModel;
...
(function() {
  define('module/design/framework/resource/AsgModel',["../ResourceModel", "../ComplexResModel", "../GroupModel", "Design", "constant", "i18n!nls/lang.js"], function(ResourceModel, ComplexResModel, GroupModel, Design, constant, lang) {
    ...
    ExpandedAsgModel = ComplexResModel.extend({
      ...
      getLc: function() {
        return this.attributes.originalAsg.get("lc");
      },
      serialize: function() {
        var layout;
        layout = this.generateLayout();
    ...
    Model = GroupModel.extend({
      ...
      serialize: function() {
        var azs, component, elbArray, elbs, expand, healthCheckType, lcId, newSubnets, sb, sbRef, subnets, _i, _j, _len, _len1, _ref;
        subnets = [this.parent()];
        _ref = this.get("expandedList");
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          expand = _ref[_i];
        ...
        if (this.get("lc")) {
          lcId = this.get('lc').createRef("LaunchConfigurationName");
        } else {
          lcId = "";
        }
        healthCheckType = "EC2";
...
(function() {
  define('module/design/framework/resource/AzModel',["../GroupModel", "./VpcModel", "constant", "i18n!nls/lang.js"], function(GroupModel, VpcModel, constant, lang) {
    ...
    Model = GroupModel.extend({
      ...
      getSubnetOfDefaultVPC: function() {
        return Model.getSubnetOfDefaultVPC(this.get("name"));
      },
      createRef: function() {
        return Model.__super__.createRef("ZoneName", true, this.id);
      },
      isCidrEnoughForIps: function(cidr) {
        var child, defaultSubnet, eni, ipCount, maxIpCount, _i, _len, _ref;
        if (!cidr) {
          defaultSubnet = this.getSubnetOfDefaultVPC();
          if (defaultSubnet) {
            cidr = defaultSubnet.cidrBlock;
          } else {
            return true;
      ...
      allPossibleAZ: function() {
        var az, azArr, azMap, azName, id, z, zones, _i, _j, _len, _len1, _ref, _ref1;
        azMap = {};
        _ref = Model.allObjects();
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          az = _ref[_i];
        ...
        if (zones) {
          ...
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            z = _ref1[_j];
            if (!azMap.hasOwnProperty(z.zoneName)) {
              azMap[z.zoneName] = "";
            }
          }
        }
...
(function() {
  define('module/design/framework/resource/CgwModel',["../ComplexResModel", "Design", "constant"], function(ComplexResModel, Design, constant) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: {
        x: 0,
...
(function() {
  define('module/design/framework/connection/SgRuleSet',["constant", "../ConnectionModel", "Design"], function(constant, ConnectionModel, Design) {
    var SgRuleSet;
...
(function() {
  define('module/design/framework/connection/SgLine',["constant", "../ConnectionModel", "../ResourceModel", "component/sgrule/SGRulePopup"], function(constant, ConnectionModel, ResourceModel, SGRulePopup) {
    var SgRuleLine;
...
(function() {
  define('module/design/framework/resource/SslCertModel',["constant", "../ComplexResModel", "../ConnectionModel"], function(constant, ComplexResModel, ConnectionModel) {
...
(function() {
  define('module/design/framework/connection/ElbAsso',["constant", "../ConnectionModel", "i18n!nls/lang.js", "Design", "component/sgrule/SGRulePopup"], function(constant, ConnectionModel, lang, Design, SGRulePopup) {
    ...
    ElbAmiAsso = ConnectionModel.extend({
      ...
      remove: function(option) {
        ...
        if (lc) {
          ...
          _ref = lc.parent().get("expandedList");
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            asg = _ref[_i];
...
(function() {
  define('module/design/framework/resource/ElbModel',["Design", "constant", "../ResourceModel", "../ComplexResModel", "./VpcModel", "./SgModel", "./SslCertModel", "../connection/SgAsso", "../connection/ElbAsso"], function(Design, constant, ResourceModel, ComplexResModel, VpcModel, SgModel, SslCertModel, SgAsso) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: function() {
        return {
      ...
      serialize: function() {
        ...
        component = {
          ...
          resource: {
            ...
            HealthCheck: {
              Interval: this.get("healthCheckInterval"),
              Target: hcTarget,
              Timeout: this.get("healthCheckTimeout"),
              HealthyThreshold: this.get("healthyThreshold"),
              UnhealthyThreshold: this.get("unHealthyThreshold")
            },
...
(function() {
  define('module/design/framework/resource/LcModel',["../ComplexResModel", "./InstanceModel", "Design", "constant", "./VolumeModel", 'i18n!nls/lang.js'], function(ComplexResModel, InstanceModel, Design, constant, VolumeModel, lang) {
    var Model, emptyArray;
    emptyArray = [];
    Model = ComplexResModel.extend({
      defaults: function() {
      ...
      groupMembers: function() {
        ...
        if (resource && resource.Instances && resource.Instances.member) {
          amis = [];
          _ref = resource.Instances.member;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            i = _ref[_i];
            amis.push({
              id: i.InstanceId,
...
(function() {
  define('module/design/framework/resource/KeypairModel',["constant", "../ComplexResModel", "../ConnectionModel"], function(constant, ComplexResModel, ConnectionModel) {
...
(function() {
  define('module/design/framework/connection/Route',["constant", "../ConnectionModel"], function(constant, ConnectionModel) {
    var C;
    C = ConnectionModel.extend({
      type: "RTB_Route",
      defaults: function() {
...
(function() {
  define('module/design/framework/connection/RtbAsso',["constant", "../ConnectionModel"], function(constant, ConnectionModel) {
    var C;
    C = ConnectionModel.extend({
      type: "RTB_Asso",
...
(function() {
  define('module/design/framework/resource/RtbModel',["../ComplexResModel", "Design", "../connection/Route", "../connection/RtbAsso", "./VpcModel", "constant", "i18n!nls/lang.js"], function(ComplexResModel, Design, Route, RtbAsso, VpcModel, constant, lang) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: {
        main: false,
...
(function() {
  define('module/design/framework/resource/IgwModel',["../ComplexResModel", "./VpcModel", "Design", "constant", "i18n!nls/lang.js"], function(ComplexResModel, VpcModel, Design, constant, lang) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: {
        x: 0,
...
(function() {
  define('module/design/framework/resource/VgwModel',["../ComplexResModel", "./VpcModel", "Design", "constant"], function(ComplexResModel, VpcModel, Design, constant) {
    var Model;
    Model = ComplexResModel.extend({
      defaults: {
        x: 0,
        ...
        return {
          component: {
            ...
            resource: {
              ...
              SubscriptionArn: this.get("appId"),
...
(function() {
  define('module/design/framework/resource/StorageModel',["Design", "../ResourceModel"], function(Design, ResourceModel) {
    var Model;
    Model = ResourceModel.extend({
      type: "AWS.Tag",
      serialize: function() {
...
(function() {
  define('module/design/framework/util/deserializeVisitor/JsonFixer',["Design"], function(Design) {
...
(function() {
  define('module/design/framework/util/deserializeVisitor/EipMerge',["Design"], function(Design) {
...
(function() {
  define('module/design/framework/util/deserializeVisitor/FixOldStack',["Design", "constant"], function(Design, constant) {
    Design.registerDeserializeVisitor(function(data, layout_data, version) {
...
(function() {
  define('module/design/framework/util/deserializeVisitor/AsgExpandor',["Design"], function(Design) {
    Design.registerDeserializeVisitor(function(data, layout_data, version) {
...
(function() {
  define('module/design/framework/util/deserializeVisitor/ElbSgNamePatch',["Design", "constant"], function(Design, constant) {
    Design.registerDeserializeVisitor(function(data, layout_data, version) {
...
(function() {
  define('module/design/framework/util/serializeVisitor/EniIpAssigner',["Design", "constant"], function(Design, constant) {
      ...
      for (comp in components) {
        ...
        switch (compo.type) {
          ...
          case 'AWS.EC2.KeyPair':
            ...
            _results.push(compo.resource.KeyName = compo.name);
            break;
          ...
          case 'AWS.IAM.ServerCertificate':
            ...
            _results.push(compo.resource.ServerCertificateMetadata.ServerCertificateName = compo.name);
            break;
          ...
          case 'AWS.SNS.Subscription':
            ...
            break;
          case "AWS.SNS.Topic":
            _results.push(compo.resource.TopicArn = "");
            break;
...
(function() {
  define('module/design/framework/canvasview/CeAz',["./CanvasElement", "constant", "CanvasManager"], function(CanvasElement, constant, CanvasManager) {
...
(function() {
  define('module/design/framework/canvasview/CeVpc',["./CanvasElement", "constant", "CanvasManager"], function(CanvasElement, constant, CanvasManager) {
...
(function() {
  define('module/design/framework/canvasview/CeAsg',["./CanvasElement", 'i18n!nls/lang.js', "constant", "Design", "CanvasManager"], function(CanvasElement, lang, constant, Design, CanvasManager) {
...
(function() {
  define('module/design/framework/canvasview/CeInstance',["i18n!nls/lang.js", "./CanvasElement", "constant", "CanvasManager", "Design"], function(lang, CanvasElement, constant, CanvasManager, Design) {
    ...
    ChildElementProto.updateAppState = function() {
      var el, instanceState, instance_data, m, stateClass, stateEl;
      m = this.model;
      if (m.design().modeIsStack() || !m.get("appId")) {
        return;
      }
    ...
    ChildElementProto.listVolume = function(appId) {
      var data, design, resource_list, v, vl, volume, _i, _len, _ref;
      vl = [];
      design = this.model.design();
      resource_list = MC.data.resource_list[design.region()];
      if (!resource_list) {
...
(function() {
  define('module/design/framework/canvasview/CeVolume',["i18n!nls/lang.js", "./CanvasElement", "constant", "CanvasManager", "event"], function(lang, CanvasElement, constant, CanvasManager, ide_event) {
...
(function() {
  define('module/design/framework/canvasview/CeLc',["./CanvasElement", "./CeInstance", "constant", "CanvasManager", 'i18n!nls/lang.js'], function(CanvasElement, CeInstance, constant, CanvasManager, lang) {
    ...
    ChildElementProto.iconUrl = function(instanceId) {
      var ami;
      if (instanceId) {
        ami = MC.data.resource_list[this.model.design().region()][instanceId];
        if (ami) {
